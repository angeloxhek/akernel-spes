...
if [ -f "$kernel" ]; then
	...
	sed -i 's/kernel.string=.*/kernel.string=AKernel-root-v1.0/g' AnyKernel3/anykernel.sh
	...
	sed -i 's/message.word=.*/message.word=Kernel with KernelSU-Next, SusFS/g' AnyKernel3/anykernel.sh
