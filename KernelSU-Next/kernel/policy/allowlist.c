...
bool ksu_uid_should_umount(uid_t uid)
{
	...
	switch (uid) {
        case 1000: // system (system_server)
        case 1005: // audio (Vendor Audio HAL)
        case 1013: // media
        case 1041: // audioserver
        case 1046: // mediacodec
            return false; // Не прячем модули от всей цепочки звука
    }
#ifdef CONFIG_KSU_DISABLE_POLICY
    return !__ksu_is_allow_uid(uid);
#else
