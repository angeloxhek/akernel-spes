...
{
	struct adapter_device *adapter_dev = get_adapter_by_name("pd_adapter");
	if (adapter_dev)
		return adapter_dev->verifed;
	return -ENOTSUPP;
}
EXPORT_SYMBOL(adapter_dev_get_pd_verified);
...
EXPORT_SYMBOL(adapter_class_exit);
